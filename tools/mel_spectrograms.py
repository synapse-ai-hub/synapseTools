...
def graph_mel_spectrogram(
    ...
    Args:
        spectrogram (np.ndarray): 2D NumPy array representing the mel spectrogram to be visualized
            (shape ``[n_mels, n_frames]``).
        output_dir (str): Directory where the spectrogram image will be saved if
            ``save`` is ``True``.
        fig_size (tuple[int, int]): Size of the figure in inches (width, height).
        name (str): Name used for the plot title and output filename (without
            extension).
